...
> = {
  info:  { bg: "bg-[#5B8DB8]/15", text: "text-[#5B8DB8]" },
  warn:  { bg: "bg-[#C9922A]/15", text: "text-[#C9922A]" },
  error: { bg: "bg-[#E05555]/15", text: "text-[#E05555]" },
  debug: { bg: "bg-[#B8C7D8]/15", text: "text-[#B8C7D8]" },
};
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-50 overflow-hidden border-t border-[#1B2D45] bg-[#0D1B2A]/98 shadow-2xl backdrop-blur-xl"
      >
        {/* ── Header bar ── */}
        <div className="flex h-11 items-center gap-2.5 border-b border-[#1B2D45] px-3.5">
          <span className="font-mono text-[11px] font-semibold uppercase tracking-widest text-[#8A9BB5]">
            Dev Console
          </span>

          <Badge
            variant="secondary"
            className="h-[18px] rounded border-0 bg-[#1B2D45] px-1.5 font-mono text-[10px] font-medium text-[#8A9BB5] hover:bg-[#1B2D45]"
          >
            {logs.length}
          </Badge>
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-6 w-6 text-[#8A9BB5] hover:bg-[#1B2D45] hover:text-[#F5F0E8]"
            onClick={clearLogs}
          >
        ...
        {!isMinimized && (
          <div
            ref={scrollRef}
            className="h-[calc(100%-44px)] overflow-y-auto [scrollbar-color:#3A4D63_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#3A4D63] [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar]:w-1"
          >
            {logs.length === 0 ? (
              <div className="flex h-full items-center justify-center text-sm text-[#5A6B80]">
                No logs yet
              </div>
            ) : (
              <div className="divide-y divide-[#1B2D45]/60">
                {logs.map((entry) => (
                  <button
                    key={entry.id}
                    type="button"
                    ...
                    className="flex w-full flex-col px-3.5 py-1.5 text-left transition-colors hover:bg-[#0F2136]"
                  >
                    <div className="flex items-center gap-2.5">
                      {/* Timestamp */}
                      <span className="shrink-0 font-mono text-[10px] tabular-nums text-[#5A6B80]">
                      ...
                      {/* Message */}
                      <span className="flex-1 truncate font-mono text-xs text-[#F5F0E8]">
                        {entry.message}
                      </span>
                    </div>
                    ...
                    {entry.data != null && expanded === entry.id && (
                      <pre className="mt-1.5 overflow-x-auto rounded bg-[#070F1A] p-2.5 font-mono text-[10px] leading-relaxed text-[#8A9BB5]">
