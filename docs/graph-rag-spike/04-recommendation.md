...
The recommendation that fits this evidence is **conditional routing**, not a wholesale backend switch. Initial heuristic shipped in `src/lib/intelligence/router.ts`: route to graph-walk when ≥1 seed entity resolves AND a multi-hop verb / preposition pattern matches (e.g. `funded by`, `operated by`, `subsidiaries of`, `connect ... to`). Otherwise vector.

Note: an earlier draft of this heuristic required ≥2 entities, which would have routed mh-01 ("projects funded by ARENA" — 1 seed entity, +2.00 win on graph-walk) to the wrong backend. The relaxed ≥1 threshold catches mh-01-class wins at the cost of potentially over-routing some single-entity queries; every routing decision is logged so we can tighten back if needed.
...
5. **Build the query router** (week 3, ~½ day's work): _shipped on `feat/learn` as `src/lib/intelligence/router.ts` + `src/app/api/learn/query/route.ts`_. Routes to graph-walk when ≥1 seed entity resolves AND a multi-hop verb / preposition pattern matches; otherwise vector. The endpoint returns `backend_used` + classification details so the Learn UI can render a debug indicator and we can mine usage logs to refine the heuristic.
